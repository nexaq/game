 ...
 **/
export function getMouseContextOffsetInCanvas(canvas: HTMLCanvasElement, event: MouseEvent) {
    const topOffsetMouse: number = event.clientY;
    const leftOffsetMouse: number = event.clientX;

    const {top: topOffsetCanvas, left: leftOffsetCanvas} = canvas.getClientRects()[0];
    ...
    const contextTopOffsetMouseInsideCanvas = (topOffsetMouseInsideCanvas) * contextHeightRatio;
    ...
    const contextLeftOffsetMouseInsideCanvas = (leftOffsetMouseInsideCanvas) * contextWidthRatio;
