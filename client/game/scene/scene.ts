...
import { Events, subscribe, trigger } from "./events";
...
export default class Scene {
  ...
  aim(event: MouseEvent) {
    ...
    const angle = Math.atan2(
      offset.top - canvas.height / 2,
      offset.left - canvas.width / 2
    );
  ...
  private updateEnemies() {
    this.enemies.forEach((enemy, enemyIndex) => {
      ...
      const distance = Math.hypot(
        this.player.getX() - enemy.getX(),
        this.player.getY() - enemy.getY()
      );

      const endGame =
        distance - enemy.getRadius() - this.player.getRadius() < 1;
      if (endGame) {
        trigger(Events.END_GAME);
      }

      this.projectiles.forEach((projectile, projectileIndex) => {
        this.projectileToucheEnemy(
          projectile,
          projectileIndex,
          enemy,
          enemyIndex
  ...
  private projectileToucheEnemy(
    ...
    const distance = Math.hypot(
      projectile.getX() - enemy.getX(),
      projectile.getY() - enemy.getY()
    );

    const objectsTouched =
      distance - enemy.getRadius() - projectile.getRadius() < 1;
